...
public class NoteGroups {
	...
	private static void readFilenamesFromProperties() {
		...
		intervalsFilename = filenames.getProperty("intervalsFile");
		chordsFilename = filenames.getProperty("chordsFile");
		scalesFilename = filenames.getProperty("scalesFile");
	}
	...
	private static Map<String, Integer> initializeIntervalMap() {
		...
		try {
			tempMap = new ReadFile(intervalsFilename).getMapWithIntegers();
		} catch (IOException ioex) {
	...
	private static Map<String, List<String>> initializeChordMap() {
		...
		try {
			tempMap = new ReadFile(chordsFilename).getMapWithList();
		} catch (IOException ioex) {
	...
	private static Map<String, List<String>> initializeScaleMap() {
		Map<String, List<String>> tempMap = null;
		try {
			tempMap = new ReadFile(scalesFilename).getMapWithList();
		} catch (IOException ioex) {
